...
namespace EduTrackOne.Application.Classes.AddInscription
{
    public class AddInscriptionCommandHandler : IRequestHandler<AddInscriptionCommand, Result<Guid>>
    {
        ...
        private readonly ILogger<AddInscriptionCommandHandler> _logger;
        public AddInscriptionCommandHandler(
            ...
            ILogger<AddInscriptionCommandHandler> logger
        ...
        {
            _logger.LogInformation("Tentative d’inscription : Élève {NoImmatricule}, Classe {ClasseId}", request.NoImmatricule, request.ClasseId);
            ...
            if (!validationResult.IsValid)
            {
                ...
                _logger.LogWarning("Échec validation inscription : {Erreurs}", errors);
            ...
            if (classe is null)
            {
                _logger.LogWarning("Classe introuvable : {ClasseId}", request.ClasseId);
            ...
            if (eleve == null)
            {
                _logger.LogWarning("Élève introuvable : {NoImmatricule}", request.NoImmatricule);
            ...
            {
                _logger.LogInformation("Élève déjà inscrit : {EleveId} dans classe {ClasseId}", eleve.Id, classe.Id);
            ...
            {
                _logger.LogWarning("Période d'inscription invalide pour Élève {EleveId} : {Erreur}", eleve.Id, periodeResult.Error);
            ...
            _logger.LogInformation("Inscription réussie : Élève {EleveId}, Classe {ClasseId}, InscriptionId {InscriptionId}",
          eleve.Id, classe.Id, inscriptionId);
